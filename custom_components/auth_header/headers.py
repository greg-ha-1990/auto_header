...
from .const import (
    CONF_USERNAME_HEADER,
    CONF_TARGET_TOKEN,
    CONF_TARGET_GUEST_USER_ID,
...
class HeaderAuthProvider(AuthProvider):
    ...
    async def async_login_flow(self, context: Optional[Dict]) -> LoginFlow:
        ...
        target_token = self.config[CONF_TARGET_TOKEN]
        target_guest_user_id = self.config[CONF_TARGET_GUEST_USER_ID]
        request = cast(Request, context.get("request"))
        request_info = await request.json()
        is_guest = False
        if(request_info != None and "redirect_uri" in request_info):
            redirect_uri = request_info["redirect_uri"]
            parsed_url = urlparse(redirect_uri)
            query_params = parse_qs(parsed_url.query)
            token = query_params.get("token", [None])[0]
            if(target_guest_user_id != None and target_guest_user_id != "" and token != None and token != "" and token == target_token):
                is_guest = True
        _LOGGER.debug("got is_guest is {}".format(is_guest))
...
class HeaderLoginFlow(LoginFlow):
    ...
    def __init__(
        ...
        self._target_guest_user_id = target_guest_user_id
    ...
    async def async_step_init(self, user_input=None) -> Dict[str, Any]:
        ...
        if user_input is not None or self._allow_bypass_login:
            ...
            if(self._is_guest):
                return await self.async_finish({"user": self._target_guest_user_id})
            return self.async_abort(reason="not_allowed")
